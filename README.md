...
### Установка из релизного архива (рекомендуется)
...
#### 1. Установка из готового релиза (рекомендуется)

```bash
# Скачать последний релиз
wget https://github.com/ikeniborn/vpn/releases/latest/download/vpn-release.tar.gz

# Создать временный каталог для установки
mkdir -p ~/vpn-install
cd ~/vpn-install
...
- Установит все необходимые компоненты и зависимости
- Настроит systemd сервисы
- Создаст конфигурационные файлы
- Установит бинарные файлы в `/usr/local/bin`
- Создаст рабочий каталог в `/opt/vpn`
