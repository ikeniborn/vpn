...
A high-performance, type-safe VPN management system written in Rust, providing comprehensive tools for managing Xray (VLESS+Reality) and Outline VPN servers. This implementation replaces the original Bash scripts with a modern, secure, and efficient alternative.
...
- **Docker Integration**: Full containerized deployment with automatic management
- **Advanced Cryptography**: X25519 key generation, Reality protocol support
- **Comprehensive Monitoring**: Real-time metrics, traffic analysis, health checks
- **Interactive CLI**: Modern command-line interface with colored output
- **High Performance**: Rust's zero-cost abstractions for optimal performance
- **Type Safety**: Compile-time guarantees for configuration validity
...
- Linux system (Ubuntu 20.04+, Debian 11+, or CentOS 8+)
...
# Create a user
vpn user create alice --protocol vless

# Get connection information
vpn user show alice --format qr
```
