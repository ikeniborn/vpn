...
pub enum AlertCondition {
    CpuUsage { threshold: f64, operator: ComparisonOperator },
    MemoryUsage { threshold: f64, operator: ComparisonOperator },
    DiskUsage { threshold: f64, operator: ComparisonOperator },
    ResponseTime { threshold_ms: u64, operator: ComparisonOperator },
    ErrorRate { threshold: f64, operator: ComparisonOperator },
    ActiveConnections { threshold: u64, operator: ComparisonOperator },
    PacketLoss { threshold: f64, operator: ComparisonOperator },
    ContainerDown { container_name: String },
    ServiceUnavailable,
    CustomMetric { metric_name: String, threshold: f64, operator: ComparisonOperator },
...
impl AlertManager {
    ...
    pub fn update_rule(&mut self, rule: AlertRule) -> Result<()> {
        if !self.rules.contains_key(&rule.id) {
            return Err(MonitorError::AlertError(
    ...
    pub async fn evaluate_rules(
        ...
        for rule in rules {
            ...
            if let Some(last_eval) = self.last_rule_evaluation.get(&rule.id) {
                if current_time.signed_duration_since(*last_eval).to_std().unwrap() < rule.cooldown_duration {
                    continue;
                }
            }
            ...
            if condition_met {
                ...
                let resolve_condition_met = match &rule.resolve_condition {
                    Some(resolve_condition) => !self.evaluate_condition(resolve_condition, metrics, health_status)?,
                    None => !condition_met,
                };
            ...
            self.last_rule_evaluation.insert(rule.id.clone(), current_time);
        }
    ...
    fn evaluate_condition(
        ...
        match condition {
            ...
            AlertCondition::ErrorRate { threshold, operator } => {
                Ok(self.compare_values(metrics.application_metrics.error_rate, *threshold, *operator))
            }
            AlertCondition::ActiveConnections { threshold, operator } => {
            ...
            AlertCondition::PacketLoss { threshold, operator } => {
                Ok(self.compare_values(metrics.network_metrics.packet_loss, *threshold, *operator))
            }
            AlertCondition::ContainerDown { container_name } => {
                let container_running = health_status.containers.iter()
                    .find(|c| c.name == *container_name)
                    .map(|c| c.status == crate::health::ServiceStatus::Healthy)
                    .unwrap_or(false);
                Ok(!container_running)
            }
            AlertCondition::ServiceUnavailable => {
                Ok(!health_status.is_healthy())
            }
            AlertCondition::CustomMetric { metric_name, threshold, operator } => {
                let metric_value = metrics.custom_metrics.get(metric_name).copied().unwrap_or(0.0);
    ...
    async fn create_alert(
        ...
        metadata.insert("cpu_usage".to_string(), metrics.system_metrics.cpu_usage.to_string());
        metadata.insert("memory_usage".to_string(), metrics.system_metrics.memory_usage.to_string());
        metadata.insert("disk_usage".to_string(), metrics.system_metrics.disk_usage.to_string());
        metadata.insert("error_rate".to_string(), metrics.application_metrics.error_rate.to_string());
    ...
    fn generate_alert_content(
        ...
        let title = match &rule.condition {
            AlertCondition::CpuUsage { threshold, .. } => {
                format!("High CPU Usage: {:.1}% (threshold: {:.1}%)", 
            ...
            AlertCondition::DiskUsage { threshold, .. } => {
                format!("High Disk Usage: {:.1}% (threshold: {:.1}%)", 
                    metrics.system_metrics.disk_usage, threshold)
            }
            AlertCondition::ResponseTime { threshold_ms, .. } => {
                format!("High Response Time: {}ms (threshold: {}ms)", 
                    metrics.application_metrics.response_time.as_millis(), threshold_ms)
            }
            AlertCondition::ErrorRate { threshold, .. } => {
                format!("High Error Rate: {:.2}% (threshold: {:.2}%)", 
        ...
        for channel in &rule.notification_channels {
            match channel {
                ...
                NotificationChannel::Webhook { url, secret } => {
                    self.send_webhook_notification(alert, url, secret.as_deref()).await?;
                }
    ...
    async fn send_email_notification(&self, alert: &Alert, _address: &str) -> Result<()> {
        ...
        println!("Email notification: {} - {}", alert.title, alert.description);
        Ok(())
    }
    
    async fn send_webhook_notification(&self, alert: &Alert, url: &str, _secret: Option<&str>) -> Result<()> {
        ...
        if !response.status().is_success() {
            return Err(MonitorError::AlertError(
                format!("Webhook notification failed: {}", response.status())
            ));
        }
    ...
    async fn send_slack_notification(&self, alert: &Alert, webhook_url: &str, channel: &str) -> Result<()> {
        let client = reqwest::Client::new();
    ...
    async fn send_discord_notification(&self, alert: &Alert, webhook_url: &str) -> Result<()> {
        ...
        if !response.status().is_success() {
            return Err(MonitorError::AlertError(
                format!("Discord notification failed: {}", response.status())
            ));
        }
    ...
    async fn send_pagerduty_notification(&self, alert: &Alert, _integration_key: &str) -> Result<()> {
        // PagerDuty notification implementation would go here
        println!("PagerDuty notification: {} - {}", alert.title, alert.description);
        Ok(())
    }
    ...
    pub async fn acknowledge_alert(&mut self, alert_id: &str, acknowledged_by: &str) -> Result<()> {
        if let Some(alert) = self.active_alerts.get_mut(alert_id) {
            ...
            alert.metadata.insert("acknowledged_by".to_string(), acknowledged_by.to_string());
    ...
    async fn auto_resolve_alerts(&mut self, rule_id: &str) -> Result<()> {
        let alerts_to_resolve: Vec<String> = self.active_alerts.iter()
