...
async fn test_health_check_system() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let check_names: Vec<&str> = health_report.checks.iter()
        .map(|c| c.name.as_str())
        .collect();
...
async fn test_metrics_persistence() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let system_metrics = metrics_collector.collect_system_metrics().await?;
    metrics_collector.save_metrics_snapshot(&system_metrics).await?;
...
async fn test_resource_usage_alerts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    alert_manager.configure_resource_alerts(cpu_threshold, memory_threshold).await?;
    
    // Simulate high resource usage
    alert_manager.check_and_create_resource_alerts(95.0, 90.0).await?;
