...
impl X25519KeyManager {
    ...
    pub fn save_keypair(keypair: &KeyPair, private_path: &Path, public_path: &Path) -> Result<()> {
    ...
    pub fn load_keypair(private_path: &Path, public_path: &Path) -> Result<KeyPair> {
    ...
    pub fn from_base64(private_base64: &str) -> Result<KeyPair> {
        let private_key = BASE64.decode(private_base64.trim())?;
