...
impl ConfigValidator {
    ...
    pub async fn validate_installation(&self, install_path: &Path) -> Result<ValidationResult> {
        ...
        self.check_installation_directory(install_path, &mut result).await;
        
        // Check Docker configuration
        self.check_docker_configuration(install_path, &mut result).await;
        
        // Check container status
        self.check_container_status(&mut result).await;
        
        // Check network configuration
        self.check_network_configuration(install_path, &mut result).await;
    ...
    async fn check_installation_directory(&self, install_path: &Path, result: &mut ValidationResult) {
        ...
        for file in &required_files {
            ...
            if !file_path.exists() {
                self.add_error(result, check_name, &format!("Required file missing: {}", file));
            }
        }
        ...
        for dir in &optional_dirs {
            ...
            if !dir_path.exists() {
                self.add_warning(result, check_name, &format!("Optional directory missing: {}", dir));
            }
        }
        
        if result.errors.is_empty() {
            self.add_pass(result, check_name, "Installation directory structure is valid");
        }
    }
        ...
        match output {
            Ok(output) => {
                if output.status.success() {
                    ...
                    self.add_error(result, check_name, &format!(
            ...
            Err(e) => {
                self.add_error(result, check_name, &format!("Failed to validate Docker Compose: {}", e));
            }
        }
    }
    ...
    async fn check_container_status(&self, result: &mut ValidationResult) {
                ...
                match self.health_checker.check_container_health(container).await {
                    Ok(health) => {
                        if health.is_running {
                            ...
                            self.add_pass(result, &format!("{} Container", container), 
                                &format!("Container {} is running and healthy", container));
                        } else {
                            self.add_error(result, &format!("{} Container", container),
        ...
        if running_containers == 0 {
            ...
            self.add_pass(result, check_name, &format!("{} containers running", running_containers));
        }
    }
    
    async fn check_network_configuration(&self, install_path: &Path, result: &mut ValidationResult) {
            ...
            for inbound in inbounds {
                if let Some(port) = inbound["port"].as_u64() {
                    ...
                    if !PortChecker::is_port_available(port) {
                        self.add_warning(result, "Port Check", 
                            &format!("Port {} appears to be in use (expected for running server)", port));
                    } else {
                        self.add_error(result, "Port Check",
        ...
        if let Some(reality_settings) = config["inbounds"][0]["streamSettings"]["realitySettings"].as_object() {
                ...
                for server_name in server_names {
                    if let Some(sni) = server_name.as_str() {
                        match SniValidator::validate_sni(sni).await {
                            Ok(true) => {
                                self.add_pass(result, "SNI Validation", 
                                    &format!("SNI domain {} is valid and reachable", sni));
                            }
                            Ok(false) => {
                                self.add_warning(result, "SNI Validation",
                                    &format!("SNI domain {} may not be optimal", sni));
                            }
                            Err(e) => {
                                self.add_error(result, "SNI Validation",
        ...
        for log_file in &log_files {
            ...
            if log_path.exists() {
                let metadata = match std::fs::metadata(&log_path) {
                    ...
                    Err(e) => {
                        self.add_error(result, check_name, &format!("Failed to read log metadata: {}", e));
                        continue;
                    }
                };
                
                if metadata.len() > 0 {
                    self.add_pass(result, check_name, &format!("Log file {} exists and has content", log_file));
                } else {
                    self.add_warning(result, check_name, &format!("Log file {} is empty", log_file));
                }
            } else {
                self.add_warning(result, check_name, &format!("Log file {} not found", log_file));
            }
        }
    }
