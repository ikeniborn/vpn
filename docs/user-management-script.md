# User Management Script for VLESS-Reality

Create a new file called `manage-vless-users.sh` with the following content:
...
function list_users() {
  ...
  if [ -f "$USERS_DB" ]; then
    ...
    jq -r '.inbounds[0].settings.clients[] | .id + "  " + (.email // "<unnamed>") + "  unknown"' "$V2RAY_CONFIG"
  fi
...
function add_user() {
  ...
  echo "  $(dirname "$0")/generate-vless-client.sh --name \"$name\""
}
...
```bash
./manage-vless-users.sh --add --name "john-smartphone"
```

### Remove a user

```bash
./manage-vless-users.sh --remove --uuid "123e4567-e89b-12d3-a456-426614174000"
```

### Export configuration for a user

```bash
./manage-vless-users.sh --export --uuid "123e4567-e89b-12d3-a456-426614174000"