...
add_user() {
    ...
    jq ".inbounds[0].settings.clients += [{\"id\": \"$USER_UUID\", \"email\": \"$USER_NAME\"}]" "$CONFIG_FILE" > "$CONFIG_FILE.tmp"
...
edit_user() {
    ...
    jq ".inbounds[0].settings.clients += [{\"id\": \"$NEW_UUID\", \"email\": \"$NEW_USER_NAME\"}]" "$CONFIG_FILE.tmp" > "$CONFIG_FILE"
    rm "$CONFIG_FILE.tmp"
...
EOL
    ...
    # Создание ссылки для подключения
    REALITY_LINK="vless://$NEW_UUID@$SERVER_IP:$SERVER_PORT?encryption=none&security=reality&sni=$SERVER_SNI&fp=chrome&pbk=$PUBLIC_KEY&sid=$SHORT_ID&type=tcp&headerType=none#$NEW_USER_NAME"
