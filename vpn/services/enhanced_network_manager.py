...
class EnhancedNetworkManager(EnhancedBaseService[None]):
    ...
    async def _restore_firewall_rules_impl(self) -> bool:
        ...
        async with aiofiles.open(self._firewall_backup) as f:
            rules = await f.read()
    ...
    @with_retry(max_attempts=3, initial_delay=1.0)
    async def get_public_ip(self, force_refresh: bool = False) -> str | None:
    ...
    async def _get_public_ip_impl(self, current_time: float) -> str | None:
    ...
    async def get_default_interface(self) -> str | None:
    ...
    async def _get_default_interface_impl(self) -> str | None:
