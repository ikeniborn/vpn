...
from vpn.core.models import ProtocolType, ServerConfig, ServerStatus
...
class ServerManager(BaseService):
    ...
    async def install(
        ...
        **extra_config
        ...
        try:
            ...
            if protocol.value == "vless" or protocol.value == "shadowsocks":
            ...
            raise ServerError(f"Failed to install server: {e!s}")
    ...
    async def uninstall(self, name: str, force: bool = False) -> None:
        ...
        try:
            ...
            raise ServerError(f"Failed to uninstall server: {e!s}")
    ...
    async def list(self, protocol: str | None = None) -> list[ServerConfig]:
        """List all servers."""
        await self._load_servers()
