...
class UserManager(CRUDService[User], EventEmitter):
    ...
    async def create(
        ...
        **kwargs
        ...
        async with get_session() as session:
            ...
            # Prepare user data for database - keep datetime objects as datetime
            user_data = {
                'id': str(user.id),
                'username': user.username,
                'email': user.email,
                'status': user.status.value,
                'protocol': user.protocol.model_dump(mode="json"),
                'keys': user.keys.model_dump(mode="json"),
                'traffic': user.traffic.model_dump(mode="json"),
                'created_at': user.created_at,  # Keep as datetime object
                'updated_at': user.updated_at,  # Keep as datetime object
                'expires_at': user.expires_at,  # Keep as datetime object
                'notes': user.notes
            }
    ...
    async def reset_traffic(self, user_id: str) -> User | None:
    ...
    async def delete_batch(self, user_ids: builtins.list[str]) -> int:
