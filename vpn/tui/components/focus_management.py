...
class FocusableWidget:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._focus_group: FocusGroup | None = None
        self._focus_ring: FocusRing | None = None
        self._tab_index: int | None = None
        self._focus_neighbors: dict[FocusDirection, Widget | None] = {}
        self._last_focused_child: Widget | None = None
    ...
    @tab_index.setter
    def tab_index(self, index: int | None) -> None:
        """Set the tab index for this widget."""
        self._tab_index = index
        if self._focus_group:
            self._focus_group._rebuild_tab_order()

    def set_focus_neighbor(self, direction: FocusDirection, widget: Widget | None) -> None:
        """Set a focus neighbor in a specific direction."""
        self._focus_neighbors[direction] = widget

    def get_focus_neighbor(self, direction: FocusDirection) -> Widget | None:
...
class FocusGroup:
    ...
    def __init__(
        ...
        self._current_focus: Widget | None = None
    ...
    def _find_spatial_neighbor(self, current: Widget, direction: FocusDirection) -> Widget | None:
        ...
        for widget in self._tab_order:
            ...
            if (direction == FocusDirection.UP and widget_region.y >= current_region.y) or (direction == FocusDirection.DOWN and widget_region.y <= current_region.y) or (direction == FocusDirection.LEFT and widget_region.x >= current_region.x) or (direction == FocusDirection.RIGHT and widget_region.x <= current_region.x):
                continue
...
class FocusRing:
    ...
    def __init__(self, name: str, config: FocusConfig | None = None):
        """Initialize focus ring."""
        self.name = name
        self.config = config or FocusConfig()
        self._groups: dict[str, FocusGroup] = {}
        self._current_group: FocusGroup | None = None
    ...
    def pop_modal(self) -> FocusGroup | None:
...
class FocusManager:
    ...
    def __init__(self, app: App, config: FocusConfig | None = None):
        """Initialize focus manager."""
        self.app = app
        self.config = config or FocusConfig()
        self._rings: dict[str, FocusRing] = {}
        self._current_ring: FocusRing | None = None
    ...
    def create_ring(self, name: str, config: FocusConfig | None = None) -> FocusRing:
    ...
    def get_ring(self, name: str) -> FocusRing | None:
    ...
    def get_screen_ring(self, screen: Screen) -> FocusRing | None:
...
_global_focus_manager: FocusManager | None = None


def get_global_focus_manager() -> FocusManager | None:
    """Get the global focus manager instance."""
    return _global_focus_manager


def initialize_focus_manager(app: App, config: FocusConfig | None = None) -> FocusManager:
