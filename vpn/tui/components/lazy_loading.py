...
from textual.widgets import Button, ProgressBar, Spinner, Static
...
class LoadingConfig:
    ...
    timeout_seconds: int | None = 30  # Loading timeout
...
class LazyLoadableWidget(Container, ABC):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._loader_worker: Worker | None = None
...
class VirtualScrollingList(LazyLoadableWidget):
    ...
    def render_item(self, index: int, item: Any) -> ComposeResult:
        ...
        yield Static(f"Item {index}: {item!s}", classes="virtual-item")
...
class LazyScreen(Screen):
    ...
    def get_section(self, section_id: str) -> LazyLoadableWidget | None:
