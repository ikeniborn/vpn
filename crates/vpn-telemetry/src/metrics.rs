...
impl MetricsCollector {
    ...
    pub async fn new(config: &TelemetryConfig) -> Result<Self> {
        ...
        let connection_duration = HistogramVec::new(
            ...
            .namespace("vpn")
            .buckets(vec![
                1.0, 5.0, 15.0, 30.0, 60.0, 300.0, 600.0, 1800.0, 3600.0,
            ]),
            &["user_id", "protocol"],
        )
        .map_err(|e| TelemetryError::MetricsError {
        ...
        let container_status = GaugeVec::new(
            prometheus::Opts::new("container_status", "Current container status").namespace("vpn"),
        ...
        // Server metrics
        let server_uptime = Gauge::new("vpn_server_uptime_seconds", "Server uptime in seconds")
            .map_err(|e| TelemetryError::MetricsError {
                message: format!("Failed to create server_uptime metric: {}", e),
            })?;

        let server_cpu_usage = Gauge::new(
            "vpn_server_cpu_usage_percent",
        ...
        let system_cpu_usage =
            Gauge::new("system_cpu_usage_percent", "System CPU usage percentage").map_err(|e| {
        ...
        let system_memory_usage =
            Gauge::new("system_memory_usage_bytes", "System memory usage in bytes").map_err(
                |e| TelemetryError::MetricsError {
    ...
    async fn collect_metrics(collector: Arc<RwLock<Self>>) -> Result<()> {
        ...
        if let Ok(system_info) = Self::get_system_info().await {
            collector_guard
                .system_cpu_usage
                .set(system_info.cpu_percent);
            collector_guard
                .system_memory_usage
                .set(system_info.memory_usage as f64);
            collector_guard
                .system_disk_usage
                .set(system_info.disk_usage as f64);
            collector_guard
                .system_network_bytes
        ...
        collector_guard
            .server_uptime
            .set(Self::get_uptime().await as f64);
    ...
    pub async fn record_metric(
        ...
        if !custom_gauges.contains_key(name) {
            let gauge = Gauge::new(name, &format!("Custom metric: {}", name)).map_err(|e| {
                TelemetryError::MetricsError {
