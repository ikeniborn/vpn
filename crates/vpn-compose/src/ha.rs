...
impl HAManager {
    ...
    pub async fn scale_for_ha(&self) -> Result<()> {
        ...
        info!("Services scaled for HA: VPN={}, API={}, Nginx={}", 
              self.config.vpn_replicas, 
              self.config.api_replicas,
              self.config.nginx_replicas);
    ...
    async fn generate_ha_compose(&self) -> Result<()> {
        ...
        generator.set_environment(&Environment::production()).await?;
        
        // Generate compose files with HA settings
        generator.generate_compose_files().await?;
        
        // Copy HA overlay file to output directory
        let ha_overlay_content = include_str!("../../../templates/docker-compose/high-availability.yml");
        let ha_path = self.compose_config.compose_dir.join("docker-compose.ha.yml");
    ...
    async fn setup_database_replication(&self) -> Result<()> {
        ...
        let output = self.compose_manager
            .exec("postgres-primary", &["bash", "/docker-entrypoint-initdb.d/init-replication.sh"])
            .await?;
    ...
    async fn setup_redis_sentinel(&self) -> Result<()> {
        ...
        let sentinel_running = status.services.iter()
    ...
    async fn setup_keepalived(&self) -> Result<()> {
        ...
        let status = self.compose_manager.get_status().await?;
        let keepalived_running = status.services.iter()
    ...
    async fn remove_ha_services(&self) -> Result<()> {
        ...
        for service in ha_services {
            let _ = self.compose_manager.exec("docker", &["stop", service]).await;
    ...
    fn check_service_health(&self, services: &[crate::manager::ServiceStatus], service_name: &str) -> Result<bool> {
        let healthy = services.iter()
    ...
    fn count_active_replicas(&self, services: &[crate::manager::ServiceStatus]) -> HashMap<String, u32> {
    ...
    fn get_expected_replicas(&self) -> HashMap<String, u32> {
        ...
        if self.config.enabled {
            ...
            if self.config.postgres_replication {
                expected.insert("postgres-replica".to_string(), self.config.postgres_replicas);
            }
        }
    ...
    async fn get_unhealthy_instances(&self, service: &str) -> Result<Vec<String>> {
        ...
        let unhealthy = status.services.iter()
    ...
    async fn start_replacement_instance(&self, service: &str) -> Result<()> {
        ...
        let current_scale = self.count_active_replicas(
            &self.compose_manager.get_status().await?.services
        ).get(service).copied().unwrap_or(0);
    ...
    async fn generate_region_compose(&self, region: &str) -> Result<()> {
        ...
        let region_config = format!(
            ...
            region, region, region, region.to_uppercase()
        );
        
        let path = self.compose_config.compose_dir.join(format!("region-{}.yml", region));
