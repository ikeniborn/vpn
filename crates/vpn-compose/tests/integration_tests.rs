use vpn_compose::{
    ComposeOrchestrator, ComposeConfig, ComposeGenerator, GeneratorOptions,
    ServiceManager, Environment, TemplateManager, TemplateContext,
    ComposeFile, ComposeError,
};
...
"#;
    ...
    tokio::fs::write(templates_dir.join("base.yml"), base_template).await.unwrap();
...
async fn test_service_dependency_ordering() {
    ...
    let postgres_pos = ordered_services.iter().position(|s| s == "postgres").unwrap();
    let redis_pos = ordered_services.iter().position(|s| s == "redis").unwrap();
    let vpn_server_pos = ordered_services.iter().position(|s| s == "vpn-server").unwrap();
    let nginx_pos = ordered_services.iter().position(|s| s == "nginx-proxy").unwrap();
...
"#;
    ...
    tokio::fs::write(config.templates_dir.join("development.yml"), dev_template).await.unwrap();
    tokio::fs::write(config.templates_dir.join("staging.yml"), staging_template).await.unwrap();
    tokio::fs::write(config.templates_dir.join("production.yml"), prod_template).await.unwrap();
    ...
    for env in ["development", "staging", "production"] {
        ...
        tokio::fs::create_dir_all(&config.compose_dir.join(env)).await.unwrap();
        ...
        let env_override = config.compose_dir.join(env).join(format!("docker-compose.{}.yml", env));
