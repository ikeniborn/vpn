...
fn test_format_utilities() {
    ...
    assert_eq!(
        format_utils::format_addr("192.168.1.1", 80),
        "192.168.1.1:80"
    );
    assert_eq!(format_utils::format_addr("::1", 80), "[::1]:80");
    assert_eq!(
        format_utils::format_addr("2001:db8::1", 443),
        "[2001:db8::1]:443"
    );

    // Test protocol info formatting
    assert_eq!(
        format_utils::format_protocol_info("vless", Some("1.8.0")),
        "VLESS v1.8.0"
    );
    assert_eq!(
        format_utils::format_protocol_info("shadowsocks", None),
        "SHADOWSOCKS"
    );
}
...
async fn test_migration_validation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::fs::write(
        valid_source.join("docker-compose.yml"),
        "version: '3'\nservices:\n  xray: {}",
    )?;
...
fn test_sanitization_utilities() {
    ...
    assert_eq!(
        validation::sanitize_filename("valid_filename.txt"),
        "valid_filename.txt"
    );
    assert_eq!(
        validation::sanitize_filename("file<>with|invalid*chars.txt"),
