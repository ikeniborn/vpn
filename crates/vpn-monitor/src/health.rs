...
impl HealthMonitor {
    ...
    pub async fn check_overall_health(&self) -> Result<HealthStatus> {
        ...
        let overall_status =
            self.calculate_overall_status(&containers, &system_metrics, &network_status);
    ...
    async fn check_network_status(&self) -> Result<NetworkStatus> {
        ...
        if let Ok(_) = tokio::net::lookup_host("google.com:80").await {
            ...
            status
                .response_times
                .insert("dns".to_string(), dns_start.elapsed().as_millis() as u64);
        }
    ...
    fn calculate_overall_status(
        ...
        let has_critical_containers = containers
            .iter()
            .any(|c| c.status == ServiceStatus::Critical);

        let has_critical_system_issues = system_metrics.cpu_usage > 95.0
        ...
        // Check for warnings
        let has_warning_containers = containers
            .iter()
            .any(|c| c.status == ServiceStatus::Warning);

        let has_warning_system_issues = system_metrics.cpu_usage > 80.0
        ...
        let healthy_containers = containers
            .iter()
    ...
    pub async fn run_health_check_command(&self, command: &str) -> Result<String> {
        ...
        if output.status.success() {
            ...
            Err(MonitorError::HealthCheckError(format!(
                "Command failed: {}",
