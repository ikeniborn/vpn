...
pub enum TransactionOp {
    ...
    Delete { key: String },
    ConditionalSet { key: String, value: Value, expected: Option<Value> },
}
...
pub async fn create_storage_backend(config: &StorageBackendConfig) -> Result<Arc<dyn DistributedConfigStorage>> {
    match config {
        StorageBackendConfig::Sled { path } => {
            Ok(Arc::new(SledStorage::new(path).await?))
        },
...
impl DistributedConfigStorage for MemoryStorage {
    ...
    async fn watch_config(&self, key: &str) -> Result<tokio::sync::mpsc::Receiver<ConfigChange>> {
        ...
        let sender = watchers.entry(key.to_string())
                ...
                TransactionOp::ConditionalSet { key, expected, .. } => {
                    ...
                    if current != expected.as_ref() {
                        return Err(ClusterError::invalid_state(
                            format!("Conditional set failed for key {}: expected {:?}, got {:?}", 
    ...
    async fn transaction(&self, ops: Vec<TransactionOp>) -> Result<()> {
        ...
        for op in &ops {
            if let TransactionOp::ConditionalSet { key, expected, .. } = op {
                ...
                if &current != expected {
                    return Err(ClusterError::invalid_state(
                        format!("Conditional set failed for key {}", key)
                    ));
                }
            }
        }
...
impl EtcdStorage {
    pub async fn new(_endpoints: &[String]) -> Result<Self> {
        ...
        Err(ClusterError::configuration("etcd storage not yet implemented"))
    }
}
...
impl ConsulStorage {
    pub async fn new(_address: &str) -> Result<Self> {
        ...
        Err(ClusterError::configuration("consul storage not yet implemented"))
    }
}
...
impl TiKVStorage {
    pub async fn new(_pd_endpoints: &[String]) -> Result<Self> {
        ...
        Err(ClusterError::configuration("tikv storage not yet implemented"))
    }
}
...
mod tests {
    ...
    async fn test_memory_storage() {
        ...
        storage.store_config("test_key", value.clone()).await.unwrap();
    ...
    async fn test_memory_storage_watch() {
        ...
        storage.store_config("watch_key", value.clone()).await.unwrap();
        
        let change = timeout(Duration::from_millis(100), watcher.recv()).await.unwrap().unwrap();
    ...
    async fn test_memory_storage_transaction() {
        ...
        // Set initial value
        storage.store_config("key1", serde_json::json!("value1")).await.unwrap();
        ...
        assert_eq!(storage.get_config("key1").await.unwrap(), Some(serde_json::json!("updated_value1")));
        assert_eq!(storage.get_config("key2").await.unwrap(), Some(serde_json::json!("value2")));
    ...
    async fn test_sled_storage() {
        ...
        storage.store_config("sled_key", value.clone()).await.unwrap();
    ...
    async fn test_create_storage_backend() {
        ...
        storage.store_config("test", serde_json::json!("value")).await.unwrap();
        ...
        sled_storage.store_config("sled_test", serde_json::json!("sled_value")).await.unwrap();
