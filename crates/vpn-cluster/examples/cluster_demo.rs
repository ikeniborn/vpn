...
use vpn_cluster::{
    ClusterManager, ClusterConfig, ClusterGrpcClient,
    Node, NodeId, config::StorageBackendConfig,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        match client.get_cluster_status(address).await {
            Ok(status) => {
                if let Some(cluster_state) = status.cluster_state {
                    println!("✅ Node {} - Cluster '{}' has {} nodes", 
                           port, cluster_state.cluster_name, cluster_state.nodes.len());
    ...
    for port in [8001, 8002, 8003] {
        ...
        match client.send_heartbeat(address, resources).await {
            Ok(response) => {
                println!("✅ Heartbeat to node {} successful (term: {})", port, response.term);
