...
impl Socks5Server {
    ...
    async fn handle_connection_inner(
        ...
        if let Err(e) = self.manager.check_rate_limit(&user_id).await {
            ...
            super::protocol::send_reply(&mut client, Reply::ConnectionNotAllowed, peer_addr)
                .await?;
            return Err(e);
        }
        ...
        match request.command {
            Command::Connect => self.handle_connect(client, request, &user_id).await,
            Command::Bind => self.handle_bind(client, request, &user_id).await,
            Command::UdpAssociate => {
                self.handle_udp_associate(client, request, &user_id, peer_addr)
                    .await
            }
        }
    }
    ...
    async fn handle_authentication(
        ...
        if auth_required && methods.contains(&AuthMethod::UserPass) {
            ...
            client
                .write_all(&[0x05, AuthMethod::UserPass as u8])
                .await?;
            ...
            match self
                .manager
                .authenticate(Some((username.clone(), password.clone())), peer_addr)
                .await
            {
                Ok(user_id) => {
                    // Send success
            ...
            client
                .write_all(&[0x05, AuthMethod::NoAcceptable as u8])
                .await?;
    ...
    async fn handle_bind(
        ...
        info!(
            "BIND listener created at {} for user {}",
            local_addr, user_id
        );
        ...
        if let AddressType::IPv4(bytes) = &request.address {
            ...
            if expected_ip != Ipv4Addr::new(0, 0, 0, 0)
                && remote_addr.ip() != IpAddr::V4(expected_ip)
            {
                return Err(ProxyError::socks5(
                    "BIND connection from unexpected address",
                ));
            }
        }
    ...
    async fn handle_udp_associate(
        ...
        info!(
            "SOCKS5 UDP ASSOCIATE request from {} for {:?}:{}",
            user_id, request.address, request.port
        );
        ...
        info!(
            "UDP relay socket created at {} for user {}",
            udp_addr, user_id
        );
...
async fn handle_udp_relay(
    ...
    let mut associations: std::collections::HashMap<SocketAddr, SocketAddr> =
