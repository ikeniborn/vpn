...
pub async fn read_auth_methods(stream: &mut TcpStream) -> Result<Vec<AuthMethod>> {
    ...
    if header[0] != 0x05 {
        return Err(ProxyError::socks5(format!("Invalid SOCKS version: {}", header[0])));
    }
...
pub async fn read_user_pass_auth(stream: &mut TcpStream) -> Result<(String, String)> {
    ...
    if header[0] != 0x01 {
        return Err(ProxyError::socks5(format!("Invalid auth version: {}", header[0])));
    }
...
pub async fn read_request(stream: &mut TcpStream) -> Result<Socks5Request> {
    ...
    let address = match atyp {
        ...
        _ => {
            return Err(ProxyError::socks5(format!("Invalid address type: {}", atyp)));
        }
    };
