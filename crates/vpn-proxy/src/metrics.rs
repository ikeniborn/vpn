...
impl ProxyMetrics {
    ...
    pub fn record_connection(&self, protocol: &str, success: bool) {
        ...
        if success {
            self.connections_active.with_label_values(&[protocol]).inc();
        }
    }

    /// Record connection closed
    pub fn record_connection_closed(&self, protocol: &str) {
        self.connections_active.with_label_values(&[protocol]).dec();
    }
...
async fn serve_metrics(metrics: Arc<ProxyMetrics>) -> impl axum::response::IntoResponse {
    use axum::body::Body;
