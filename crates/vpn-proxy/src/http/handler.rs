...
impl HttpProxy {
    ...
    async fn handle_connection_inner(
        ...
        loop {
            ...
            debug!("HTTP {} request from {} to {}", request.method.as_str(), peer_addr, request.uri);
            
            // Authenticate if required
            let user_id = match self.manager.authenticate(request.proxy_auth(), peer_addr).await {
                Ok(id) => id,
                Err(e) => {
    ...
    async fn handle_http_request(
        ...
        debug!("HTTP {} request from {} to {}", request.method.as_str(), user_id, target_addr);
        ...
        self.forward_http_response(&mut upstream, client, user_id).await?;
