use tokio;
use vpn_docker::{ContainerConfig, ContainerStats, ContainerStatus, DockerManager};
...
fn test_container_status_conversion() {
    ...
    assert!(matches!(
        ContainerStatus::from("stopped"),
        ContainerStatus::Stopped
    ));
    assert!(matches!(
        ContainerStatus::from("paused"),
        ContainerStatus::Paused
    ));
    assert!(matches!(
        ContainerStatus::from("exited (0)"),
        ContainerStatus::Exited(0)
    ));
    assert!(matches!(
        ContainerStatus::from("exited (1)"),
        ContainerStatus::Exited(1)
    ));
    assert!(matches!(
        ContainerStatus::from("unknown_status"),
...
fn test_container_status_display() {
    ...
    assert_eq!(
        format!("{}", ContainerStatus::Error("test".to_string())),
        "error: test"
    );
