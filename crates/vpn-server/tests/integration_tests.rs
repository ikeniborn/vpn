...
use tokio;
use vpn_server::{
    ConfigValidator, DockerComposeTemplate, KeyRotationManager, ProxyInstaller, ServerInstaller,
    ServerLifecycle,
};
...
async fn test_installation_manager() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let workflow = install_manager
        .create_installation_workflow(&install_path, VpnProtocol::Vless)
        .await?;
...
async fn test_template_manager() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vless_template = template_manager
    ...
    let vmess_template = template_manager
        .generate_vmess_config(8080, "test_uuid", "test_alter_id")
        .await?;
...
async fn test_docker_compose_generation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let docker_compose = template_manager
        .generate_docker_compose(VpnProtocol::Vless, "/opt/vpn/config", "/opt/vpn/logs", 8443)
        .await?;
...
fn test_protocol_config_variants() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for protocol in protocols {
        ...
        match (&protocol, &deserialized) {
            ...
            (
                ProtocolConfig::Trojan { password: p1, .. },
                ProtocolConfig::Trojan { password: p2, .. },
            ) => {
                assert_eq!(p1, p2);
            }
            (
                ProtocolConfig::Shadowsocks { method: m1, .. },
                ProtocolConfig::Shadowsocks { method: m2, .. },
            ) => {
                assert_eq!(m1, m2);
            }
...
async fn test_multi_protocol_support() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for protocol in protocols {
        let template = template_manager
            .generate_protocol_template(&protocol)
            .await?;
