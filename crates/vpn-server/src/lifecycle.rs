...
impl ServerLifecycle {
    ...
    pub async fn reload_config(&self, install_path: &Path) -> Result<()> {
        ...
        for container in &containers {
            if self.container_manager.container_exists(container).await {
                ...
                match self.container_manager.exec_command(
                    container,
                    vec!["killall", "-HUP", "xray"]
                ).await {
    ...
    async fn get_container_uptime(&self, container: &str) -> Result<Duration> {
        ...
        Err(ServerError::LifecycleError("Could not determine uptime".to_string()))
    }
    ...
    pub async fn restore_configuration(&self, install_path: &Path, backup_path: &Path) -> Result<()> {
        ...
        if !validation_result.is_valid {
            return Err(ServerError::ValidationError(
                format!("Restored configuration is invalid: {:?}", validation_result.errors)
            ));
        }
