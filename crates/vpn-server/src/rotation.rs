...
impl KeyRotationManager {
        ...
        if options.rotate_server_keys {
            match self.rotate_server_keys(install_path).await {
                ...
                Err(e) => {
                    result
                        .failed_rotations
                        .insert("server".to_string(), e.to_string());
                }
            }
        }
            ...
            for inbound in inbounds {
                if let Some(stream_settings) = inbound["streamSettings"].as_object_mut() {
                    if let Some(reality_settings) =
                        stream_settings["realitySettings"].as_object_mut()
                    {
                        reality_settings["privateKey"] =
                            serde_json::Value::String(private_key.trim().to_string());
                    }
                }
            }
        }
    ...
    async fn create_backup(&self, install_path: &Path) -> Result<String> {
        ...
        self.server_lifecycle
            .backup_configuration(install_path, &backup_path)
            .await?;
    ...
    pub async fn schedule_automatic_rotation(
        ...
        println!(
            "Automatic key rotation scheduled every {} days",
            interval_days
        );
    ...
    pub async fn validate_keys(&self, install_path: &Path) -> Result<ValidationStatus> {
            ...
            match key_manager.from_base64(&private_key) {
                Ok(keypair) => {
                    if keypair.public_key_base64() == public_key.trim() {
                        ...
                        status
                            .issues
                            .push("Server public key doesn't match private key".to_string());
                    }
                }
                Err(e) => {
                    status
                        .issues
                        .push(format!("Invalid server private key: {}", e));
                }
            }
        } else {
        ...
        for user in users {
            if let (Some(private_key), Some(public_key)) =
                (&user.config.private_key, &user.config.public_key)
            {
                let key_manager = X25519KeyManager::new();
                match key_manager.from_base64(private_key) {
                    Ok(keypair) => {
                        if keypair.public_key_base64() == *public_key {
                            ...
                            status
                                .issues
                                .push(format!("User {} has mismatched keys", user.name));
                        }
                    }
                    Err(_) => {
                        status
                            .issues
                            .push(format!("User {} has invalid private key", user.name));
                    }
                }
            } else {
                status
                    .issues
                    .push(format!("User {} missing keys", user.name));
            }
        }
...
impl ValidationStatus {
    pub fn is_fully_valid(&self) -> bool {
        self.server_keys_valid && self.user_keys_valid == self.total_users && self.issues.is_empty()
    }

    pub fn validation_percentage(&self) -> f64 {
        if self.total_users == 0 {
            if self.server_keys_valid {
                100.0
            } else {
                0.0
            }
