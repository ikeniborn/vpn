...
use vpn_users::{
    UserManager, User, UserConfig, UserStatus, UserStats, VpnProtocol
};
...
async fn test_user_listing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _user2 = user_manager.create_user("user2".to_string(), VpnProtocol::Outline).await?;
    let _user3 = user_manager.create_user("user3".to_string(), VpnProtocol::Wireguard).await?;
