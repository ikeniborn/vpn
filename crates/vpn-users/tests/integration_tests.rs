...
async fn test_batch_user_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let update_result = batch_ops.update_user_status(&user_ids, UserStatus::Suspended).await?;
...
async fn test_user_search_and_filtering() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vless_users = user_manager.filter_users_by_protocol(VpnProtocol::Vless).await?;
    assert_eq!(vless_users.len(), 1);
    assert_eq!(vless_users[0].protocol, VpnProtocol::Vless);
    
    // Test filter by status
    let suspended_users = user_manager.filter_users_by_status(UserStatus::Suspended).await?;
