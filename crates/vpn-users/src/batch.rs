...
impl BatchOperations {
    ...
    pub async fn create_progress_tracker(
        &self,
        operation_id: String,
        total_items: usize,
        can_resume: bool,
    ) -> (ProgressTracker, ProgressReceiver) {
    ...
    pub async fn get_progress(&self, operation_id: &str) -> Option<ProgressInfo> {
        ...
        trackers
            .get(operation_id)
            .map(|tracker| tracker.get_progress())
    }
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                if let Ok(checkpoint_json) = std::fs::read_to_string(&path) {
                    if let Ok(checkpoint) =
                        serde_json::from_str::<BatchOperationCheckpoint>(&checkpoint_json)
                    {
    ...
    pub async fn create_multiple_users(
        ...
        self.create_multiple_users_with_progress(request, None)
            .await
    }
    ...
    pub async fn create_multiple_users_with_progress(
        ...
        let (tracker, _receiver) = self
            .create_progress_tracker(operation_id.clone(), total_items, true)
            .await;
    ...
    pub async fn validate_batch_create(
        &self,
        request: &BatchCreateRequest,
                ...
                for (i, email) in emails.iter().enumerate() {
                    if !email.contains('@') || !email.contains('.') {
                        errors.push(format!(
                            ...
                            request.names.get(i).unwrap_or(&"unknown".to_string()),
                            email
                        ));
                    }
                }
                        ...
                        if let Err(e) = self.user_manager.update_user(user).await {
                            ...
                            if enable_rollback {
                                if let Err(rollback_err) =
                                    self.user_manager.delete_user(&user_id).await
                                {
                                    eprintln!(
                                        "Warning: Failed to rollback user creation for {}: {}",
                                        name, rollback_err
                                    );
                                }
                                if let Some(ref mut rb) = rollback_info {
    ...
    pub async fn rollback_operation(
        ...
        Ok(BatchOperationResult {
            successful,
            failed,
            total_processed: rollback_info.created_users.len()
                + rollback_info.modified_users.len()
                + rollback_info.deleted_users.len(),
            duration_ms: 0, // Will be set by caller
            progress_info: None,
        })
    }

    pub async fn delete_multiple_users(
        &self,
        user_ids: Vec<String>,
    ) -> Result<BatchOperationResult> {
        let mut successful = Vec::new();
        ...
        for mut user in imported_users {
            ...
            if options.validate_configs {
                if user.config.private_key.is_none() || user.config.public_key.is_none() {
                    failed.insert(user.name.clone(), "Missing cryptographic keys".to_string());
                    continue;
                }
            }
