...
impl UserManager {
        ...
        if !storage_path.exists() {
            if let Err(e) = fs::create_dir_all(&storage_path) {
                if e.kind() == std::io::ErrorKind::PermissionDenied {
                    eprintln!(
                        "Warning: Cannot create storage directory ({}): Permission denied",
                        storage_path.display()
                    );
                    eprintln!(
                        "Running in read-only mode. User management operations will be limited."
                    );
                    read_only_mode = true;
                } else {
    ...
    pub async fn list_users(&self, options: Option<UserListOptions>) -> Result<Vec<User>> {
        let mut user_list: Vec<User> = self
            .users
            .iter()
            .map(|entry| entry.value().clone())
            .collect();
        ...
        match options.sort_by {
            ...
            SortBy::LastActive => user_list.sort_by(|a, b| match (a.last_active, b.last_active) {
            ...
            SortBy::TotalTraffic => {
                user_list.sort_by(|a, b| b.total_traffic().cmp(&a.total_traffic()))
            }
        }
        ...
        let entries = match fs::read_dir(&users_dir) {
            ...
            Err(e) => {
                if e.kind() == std::io::ErrorKind::PermissionDenied {
                    ...
                    eprintln!(
                        "Warning: Cannot access users directory ({}): Permission denied",
                        users_dir.display()
                    );
                    return Ok(());
                } else {
